class IBatteryLife {
    private chargeTimes: number = -1;
    private designCapacity: number = -1;
    private actualCapacity: number = -1;
    private size = 150;
    private textIntro = {
        reduced: 'Reduced',
        remaining: 'Remaining',
    ...
    private colors = ['#1B9E77', '#D95F02'];
    private debug = false;

    setBatteryInfo(chargeTimes: number, designCapacity: number, actualCapacity: number) {
    ...
    setTextIntro(reduced: string, remaining: string, chargeTimes: string) {
    ...
    showChart(selector: string, callback?: { (imgURL?: string): void }) {
        ...
        chart.selectAll('path')
            ...
            .append('g')
            .append('path')
            .attr('d', <string><any>(arc))
            .style('fill', function (d) {
                return colors[d.index]
            });
        let formatNumber = d3.format('.2f');
        let lifeCalc = (this.actualCapacity / this.designCapacity) * 100;
        let data: Array<string> = [formatNumber(lifeCalc) + '%', formatNumber(100 - lifeCalc) + '%', '' + this.chargeTimes];
        // life text
        let texts = chart.append('g');
        let newIntro: Array<string> = [];
