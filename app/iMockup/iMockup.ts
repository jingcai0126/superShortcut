...
class IMockup {
    ...
    private device = MockDevices.IPhone;
    private mockType = MockType.flat;
    private model = this._flat_iPhones[0];
    private screenColor = "black";

    getModels(devices: MockDevices, mockType: MockType): Array<string> {
        switch (devices) {
    ...
    setDevice(device: MockDevices, mockType: MockType, model: string) {
        this.device = device;
    ...
    setScreenColor(hexColor: string) {
        this.screenColor = hexColor;
        return this
    }

    private svgModel_1 = function (body: any, svgURL: string, imgURL: string, screenColor: string, x: number, y: number, width: number, height: number) {
        d3.svg(svgURL)
            .then(function (xml) {
                body.node()?.appendChild(xml.documentElement);
    ...
    private svgModel_2 = function (body: any, svgURL: string, imgURL: string, screenColor: string) {
        d3.svg(svgURL)
            .then(function (xml) {
                body.node()?.appendChild(xml.documentElement);
    ...
    mockup(selector: string, imgURL?: string) {
